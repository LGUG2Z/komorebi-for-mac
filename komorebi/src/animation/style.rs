...
mod tests {
    ...
    fn test_linear_boundaries() {
        assert!((Linear::evaluate(0.0) - 0.0).abs() < f64::EPSILON);
        assert!((Linear::evaluate(1.0) - 1.0).abs() < f64::EPSILON);
        assert!((Linear::evaluate(0.5) - 0.5).abs() < f64::EPSILON);
    }

    #[test]
    fn test_ease_in_out_boundaries() {
        // All ease functions should return 0 at t=0 and 1 at t=1
        assert!((EaseInSine::evaluate(0.0) - 0.0).abs() < 1e-10);
        assert!((EaseInSine::evaluate(1.0) - 1.0).abs() < 1e-10);
        assert!((EaseOutSine::evaluate(0.0) - 0.0).abs() < 1e-10);
        assert!((EaseOutSine::evaluate(1.0) - 1.0).abs() < 1e-10);
        assert!((EaseInOutSine::evaluate(0.0) - 0.0).abs() < 1e-10);
        assert!((EaseInOutSine::evaluate(1.0) - 1.0).abs() < 1e-10);
    }

    #[test]
    fn test_ease_in_out_symmetry() {
        // EaseInOut functions should be symmetric around (0.5, 0.5)
        let t = 0.25;
        let ease_in_out_at_t = EaseInOutQuad::evaluate(t);
        let ease_in_out_at_1_minus_t = EaseInOutQuad::evaluate(1.0 - t);
        assert!((ease_in_out_at_t + ease_in_out_at_1_minus_t - 1.0).abs() < 1e-10);
    }

    #[test]
    fn test_cubic_bezier_linear() {
        // CubicBezier with control points (0,0) and (1,1) should be approximately linear
        ...
        for i in 0..=10 {
            let t = i as f64 / 10.0;
            let result = bezier.evaluate(t);
            assert!(
                (result - t).abs() < 0.1,
                "CubicBezier linear failed at t={}",
                t
            );
        }
    }
    ...
    #[test]
    fn test_apply_ease_func() {
        assert!((apply_ease_func(0.5, AnimationStyle::Linear) - 0.5).abs() < f64::EPSILON);
        assert!((apply_ease_func(0.0, AnimationStyle::EaseInQuad) - 0.0).abs() < f64::EPSILON);
        assert!((apply_ease_func(1.0, AnimationStyle::EaseOutQuad) - 1.0).abs() < f64::EPSILON);
    }
}
