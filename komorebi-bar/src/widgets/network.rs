...
impl Network {
    ...
    fn default_interface(&mut self) -> String {
        let current = self.default_interface.lock().clone();
        let now = Instant::now();
    ...
    fn update_network_activity_async(&mut self) {
        ...
        thread::spawn(move || {
            ...
            if let Ok(interface) = netdev::get_default_interface()
                && let Some(_friendly_name) = &interface.friendly_name
            {
                let mut networks = networks_network_activity_arc.lock();
            ...
            // Only update if this is the latest request
            if gen_ == gen_arc.load(Ordering::SeqCst) {
                let mut act = activity_arc.lock();
                *act = activity;

                let mut tot = total_activity_arc.lock();
                *tot = total_activity;
            }
        });
    }
    ...
    fn network_activity(&mut self) -> (Vec<NetworkReading>, Vec<NetworkReading>) {
        let now = Instant::now();
        let should_update = {
            let last_update_request = self.last_update_request_network_activity.lock();
        ...
        if should_update {
            {
                let mut last_updated = self.last_update_request_network_activity.lock();
                *last_updated = now;
            }
    ...
    fn get_network_activity(&self) -> (Vec<NetworkReading>, Vec<NetworkReading>) {
        let activity = self.last_state_activity.lock().clone();
        let total_activity = self.last_state_total_activity.lock().clone();
    ...
    fn show_frame<R>(
        ...
        if SelectableFrame::new_auto(selected, auto_focus_fill)
            ...
            .clicked()
            && let Err(error) = Command::new("open")
                .args(["x-apple.systempreferences:com.apple.Network-Settings.extension"])
                .spawn()
        {
            eprintln!("{error}");
        }
