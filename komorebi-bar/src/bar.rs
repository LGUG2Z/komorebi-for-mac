...
impl Komobar {
    pub fn apply_config(
        ...
        if let Some(monitor_index) = self.monitor_index {
                ...
                && new_rect != prev_rect
            {
                ...
                // only want to send the work area offset on non-built-in displays
                if let Some(monitor_info) = self.monitor_info.as_ref()
                    && monitor_info.borrow().device.contains("Built-in")
                {
                    should_adjust = false;
                }
    ...
    pub fn position_bar(&self, ctx: &Context) {
        ...
        ctx.send_viewport_cmd(ViewportCommand::InnerSize(Vec2::new(
            self.size_rect.right as f32,
            self.size_rect.bottom as f32,
        )));
    }
    ...
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        ...
        match self.rx_gui.try_recv() {
            ...
            Ok(KomorebiEvent::Notification(notification)) => {
                ...
                if matches!(notification.event, NotificationEvent::Monitor(_)) {
